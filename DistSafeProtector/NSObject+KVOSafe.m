...
//#define DistKVOSafeLog(fmt, ...) NSLog(fmt,##__VA_ARGS__)
#define DistKVOSafeLog(fmt, ...)


@interface DistKVOObserverInfo()
...
@implementation DistKVOObserverInfo
...
@property (nonatomic,weak) DistKVOObserverInfo *safe_willRemoveObserverInfo;
...
- (void)setSafe_willRemoveObserverInfo:(DistKVOObserverInfo *)safe_willRemoveObserverInfo {
...
- (DistKVOObserverInfo *)safe_willRemoveObserverInfo {
...
- (void)safe_observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context {
    @try{
        ...
        DistSafeProtectionCrashLog(exception,DistSafeProtectorCrashTypeKVO);
    }@finally{
...
- (void)safe_addObserver:(NSObject *)observer forKeyPath:(NSString *)keyPath options:(NSKeyValueObservingOptions)options context:(void *)context {
    ...
    [lock lock];
    DistKVOObserverInfo *info = [self safe_canAddOrRemoveObserverWithKeypathWithObserver:observer keyPath:keyPath context:context haveContext:YES isAdd:YES];
    if (info != nil) {
        //如果添加过了直接return
        DistKVOSafeLog(@"添加失败:%d %@:%p safe_addObserver %@:%p  keyPath:%@",(context!=NULL),[self class],self,[observer class],observer,keyPath);
        [lock unlock];
        return;
    }
    @try {
        DistKVOSafeLog(@"添加成功:%d %@:%p safe_addObserver %@:%p  keyPath:%@",(context!=NULL),[self class],self,[observer class],observer,keyPath);
        
        NSString *targetAddress=[NSString stringWithFormat:@"%p",self];
        NSString *observerAddress=[NSString stringWithFormat:@"%p",observer];
        DistKVOObserverInfo *info = [DistKVOObserverInfo new];
        info.target=self;
...
- (void)safe_allRemoveObserver:(NSObject *)observer forKeyPath:(NSString *)keyPath context:(void *)context isContext:(BOOL)isContext {
    ...
    NSLock *lock = [[NSLock alloc]init];
    [lock lock];
    DistKVOObserverInfo *info = [self safe_canAddOrRemoveObserverWithKeypathWithObserver:observer keyPath:keyPath context:context haveContext:isContext isAdd:NO];
    if (info==nil) {
        // 重复删除观察者或不含有 或者keypath=nil  observer=nil
        DistKVOSafeLog(@"移除失败:%d %@:%p safe_removeObserver %@:%p  keyPath:%@",isContext,[self class],self,[observer class],observer,keyPath);
        [lock unlock];
        return;
    }
    
    @try {
        DistKVOSafeLog(@"移除成功:%d %@:%p safe_removeObserver %@:%p  keyPath:%@",isContext, [self class],self,[observer class],observer,keyPath);
        if (isContext) {
            NSString *targetAddress=[NSString stringWithFormat:@"%p",self];
            NSString *observerAddress=[NSString stringWithFormat:@"%p",observer];
...
- (void)safe_removeSuccessObserver:(NSObject*)observer info:(DistKVOObserverInfo *)info {
...
- (DistKVOObserverInfo *)safe_canAddOrRemoveObserverWithKeypathWithObserver:(NSObject *)observer keyPath:(NSString*)keyPath context:(void*)context haveContext:(BOOL)haveContext isAdd:(BOOL)isAdd {
    ...
    __block DistKVOObserverInfo *info;
    [downArray enumerateObjectsUsingBlock:^(DistKVOObserverInfo *  obj, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    if (contextIsNULL) {
        if (isAdd) {
            [downKeypathArray enumerateObjectsUsingBlock:^(DistKVOObserverInfo * obj, NSUInteger idx, BOOL * _Nonnull stop) {
                if (obj.context == NULL){
                    info = obj;
                    *stop = YES;
                }
...
- (void)safe_KVODealloc {
    DistKVOSafeLog(@"%@  safe_KVODealloc",[self class]);
    ...
    for (DistKVOObserverInfo *info in self.safe_upObservedArray) {
    ...
    for (DistKVOObserverInfo *upInfo in newUpArray) {
    ...
    for (DistKVOObserverInfo *downInfo in downNewArray) {
...
+ (void)safe_dealloc_crash:(NSString*)classAddress {
    NSDictionary *crashDic = KVOSafeDeallocCrashes()[classAddress];
    for (NSMutableDictionary *dic in crashDic[@"keyPaths"]) {
        ...
        NSException *exception = [NSException exceptionWithName:@"KVO crash" reason:reason userInfo:nil]; DistSafeProtectionCrashLog(exception,DistSafeProtectorCrashTypeKVO);
    }
